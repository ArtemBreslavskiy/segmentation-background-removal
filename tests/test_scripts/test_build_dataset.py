...
class TestBuildDataset:
    ...
    def test_build_processed_dataset_success(
        ...
        all_images = sorted(sample_files.DUTS_TR_IMAGES.glob("*.jpg"), key=lambda x: x.stem)
        all_masks = sorted(sample_files.DUTS_TR_MASKS.glob("*.png"), key=lambda x: x.stem)
        ...
        logger.info.assert_any_call("Processed data path: %s", sample_files.PROCESSED_DATA)

        total_files = (
            len(train_images) * 2 + len(val_images) * 2 + len(list(sample_files.DUTS_TE_IMAGES.glob("*.jpg"))) * 2
        )
    ...
    @patch("scripts.build_dataset.ProjectPaths")
    def test_build_processed_dataset_raw_not_exists(self, mock_project_paths_class, mock_paths, mock_logger):
        ...
        logger.critical.assert_called_once_with("Dataset building failed. Check error log for details.")
    ...
    def test_build_processed_dataset_mismatch_train_count(
        ...
        logger.exception.assert_any_call("Mismatch in train/val dataset: 10 images vs 9 masks")
        assert logger.critical.call_count == 2
        logger.critical.assert_any_call("Dataset building failed. Check error log for details.")
    ...
    def test_build_processed_dataset_mismatch_test_count(
        ...
        logger.exception.assert_any_call("Mismatch in test dataset: 5 images vs 4 masks")
        assert logger.critical.call_count == 2
        logger.critical.assert_any_call("Dataset building failed. Check error log for details.")
    ...
    def test_build_processed_dataset_filename_mismatch(
        ...
        (sample_files.DUTS_TR_MASKS / "img9.png").rename(sample_files.DUTS_TR_MASKS / "wrong.png")
        mock_project_paths_class.return_value = sample_files
        logger = mock_logger

        with pytest.raises(ValueError, match="Filename inconsistencies found in dataset"):
            build_processed_dataset(logger)
